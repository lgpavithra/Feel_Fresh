...
//   
    ...
    private void loadCustomers() {
     ...
     try{
       ...
       ResultSet resultSet = MYSQL.executeSearch("SELECT * FROM `product` INNER JOIN `category` ON `product`.`category_id`=`category`.`id` INNER JOIN `brand` ON `brand`.`id`=`product`.`brand_id`");
